...
<script lang="ts">
  import {
    ...
    type TypingSession as SessionData,
    type StudentResult,
  } from '../../connections';
    ...
    <form on:submit|preventDefault={handleCreateSession} class="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          disabled={isSubmitting}
        />
        <p class="mt-2 text-sm text-text-muted">Leer lassen, um ohne Zeitlimit zu starten.</p>
      </div>
...
{:else if viewMode === 'success'}
  <div class="space-y-8">
    <div class="p-8 bg-surface-elevated rounded-lg text-center">
      ...
      <p class="text-text-muted mb-6">Teilen Sie diesen Code mit Ihren Schülern:</p>
...
{:else if viewMode === 'results' && loadedSession}
  ...
  <div class="space-y-6">
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-surface-elevated">
              ...
              <th class="p-3 text-center text-text font-medium border-b border-border"
                >Erfolgreich</th
              >
              ...
              <th class="p-3 text-center text-text font-medium border-b border-border"
                >Genauigkeit</th
              >
              <th class="p-3 text-left text-text font-medium border-b border-border"
                >Abgeschlossen</th
              >
            </tr>
          </thead>
          <tbody>
            {#each sessionResults as result}
              <tr class="hover:bg-surface-elevated transition-colors">
                ...
                <td class="p-3 text-center text-success border-b border-border"
                  >{result.success_count}</td
                >
                <td class="p-3 text-center text-error border-b border-border"
                  >{result.failure_count}</td
                >
                <td class="p-3 text-center text-primary border-b border-border"
                  >{result.accuracy}%</td
                >
    ...
    <details class="bg-surface rounded-lg">
      <summary
        class="p-4 cursor-pointer text-text hover:bg-surface-elevated rounded-lg transition-colors"
      >
        Übungstext anzeigen
      </summary>
      <div class="p-4 pt-0">
